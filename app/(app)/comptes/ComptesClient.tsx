...
export function ComptesClient({ companies }: { companies: Company[] }) {
  ...
  const filtered = companies.filter((c) => {
    ...
    const matchSearch = !q || c.name.toLowerCase().includes(q) || (c.siren ?? "").includes(q) || (c.city ?? "").toLowerCase().includes(q) || (c.region ?? "").toLowerCase().includes(q);
  ...
  const tabLabel: Record<Status | "ALL", string> = {
    ALL: `Tous (${counts.ALL})`, PROSPECT: `Prospects (${counts.PROSPECT})`,
    LEAD: `Leads (${counts.LEAD})`, CLIENT: `Clients (${counts.CLIENT})`, LOST: `Perdus (${counts.LOST})`,
  };

  const tabColor: Record<Status | "ALL", string> = {
    ALL: "bg-navy text-white border-navy", PROSPECT: "bg-navy text-white border-navy",
    LEAD: "bg-teal text-white border-teal", CLIENT: "bg-gold text-white border-gold",
    ...
    <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex gap-1.5 flex-wrap">
          {ALL_STATUSES.map((s) => (
            <button key={s} onClick={() => setStatus(s)}
              className={`px-3 py-1.5 text-[12px] font-semibold rounded-lg border transition-colors ${statusFilter === s ? tabColor[s] : "bg-surface border-border text-text-2 hover:bg-bg"}`}>
              {tabLabel[s]}
            </button>
          ))}
        </div>
      ...
      <div className="bg-surface rounded-xl border border-border shadow-sm overflow-hidden">
        {filtered.length === 0 ? (
          <div className="px-5 py-12 text-center text-[13px] text-text-3">
            {companies.length === 0 ? "Aucun compte — importez vos premières entreprises depuis la Prospection." : "Aucun compte correspondant à cette recherche."}
          </div>
        ) : (
                ...
                return (
                  <tr key={c.id} className="border-b border-border last:border-0 hover:bg-bg/50 transition-colors">
                    <td className="px-3 py-3">
                      <Link href={`/comptes/${c.id}` as never} className="text-[13px] font-semibold text-navy hover:underline">{c.name}</Link>
                      <div className="text-[11px] text-text-3 font-mono">{c.siren ?? "—"} · {c.legalForm ?? "—"}</div>
                    </td>
                    <td className="px-3 py-3"><Chip color={status.color}>{status.label}</Chip></td>
      ...
      {filtered.length > 0 && (
        <p className="mt-3 text-[11px] text-text-3 text-right">
          {filtered.length} compte{filtered.length > 1 ? "s" : ""}{filtered.length !== companies.length && ` sur ${companies.length}`}
        </p>
      )}
    </div>
  );
